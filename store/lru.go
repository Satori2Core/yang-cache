...
// lruCache 是基于标准库 list（双向链表）的 LRU 缓存实现
...
func newLRUCache(opts Options) *lruCache {
	// 设置默认清理时间间隔
	cheanupInterval := opts.CleanupInterval
	if cheanupInterval <= 0 {
		cheanupInterval = time.Minute
	}
	...
	c := &lruCache{
		...
		cleanupInterval: cheanupInterval,
...
// evict 清理过期和超过内存限制的缓存，调用此方法前必须持有锁
func (c *lruCache) evict() {
	...
	// 根据内存限制清理最久未使用的项
	if c.maxBytes > 0 && c.usedBytes > c.maxBytes && c.list.Len() > 0 {
		// 链表头：最久未使用的项
		elem := c.list.Front()
		if elem != nil {
			c.removeElement(elem)
...
// GetWichExpiration 获取缓存项及其剩余过期时间
...
// UndateExpiration 更新过期时间
func (c *lruCache) UndateExpiration(key string, expiration time.Duration) bool {
	c.mu.Lock()
	defer c.mu.Unlock()
...
// MaxBtyes 返回最大允许字节数
func (c *lruCache) MaxBtyes() int64 {
	c.mu.RLock()
	defer c.mu.RUnlock()
	return c.maxBytes
}

// SetMaxBytes 设置最大的允许字节数并触发淘汰
